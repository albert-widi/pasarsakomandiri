...
func UserGetByUsername(username string) (User, error) {
	var user = User{}
	err := database.Db.Get(&user, "SELECT id, username, password, level, role, description, created_date FROM user WHERE username=?", username)
	return user, err
}
...
func UserCreateNew(user User) (sql.Result, error) {
	result, err := database.Db.Exec("INSERT INTO user(username, password, level, role, description, created_by, created_date) VALUES(?, ?, ?, ?, ?, ?, ?)", user.Username, user.Password, user.Level, user.Role, user.Description, user.Created_by, user.Created_date)
	return result, err
}
